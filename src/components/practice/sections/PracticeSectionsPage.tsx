...
export default function PracticeSectionsPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      setBusy(true);
      try {
        const r = await fetch("/api/practice/sections", {
          cache: "no-store",
        ...
        if (!contentType.includes("application/json")) {
          throw new Error(
            `Expected JSON but got "${contentType}". Body: ${raw.slice(
              0,
              200
            )}`
          );
    ...
    <div className="min-h-screen p-4 md:p-6 bg-[radial-gradient(1200px_700px_at_20%_0%,#151a2c_0%,#0b0d12_50%)] text-white/90">
      <div className="mx-auto max-w-5xl">
                          ...
                          <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              <Link
                                ...
                                href={`/practice/history?section=${encodeURIComponent(
                                  s.slug
                                )}`}
                              >
                                View history
        ...
        <div className="mt-4 text-xs text-white/50">
          Tip: “Start” passes <code>section</code>, <code>difficulty</code>, and{" "}
          <code>topic</code>. “Open module review” goes to{" "}
          <code>/practice/review/module-X</code>.
        </div>
      </div>
    </div>
  );
